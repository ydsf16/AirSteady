...
bool YoloSegDetector::Init(std::string* err) {
  if (impl_->initialized == true) {
    LOG(WARNING) << "Yolo detector has been initialized!!!";
    return true;
  }
...
bool YoloSegDetector::Initialized() {
...
std::string YoloSegDetector::ClassName(int class_id) const {
  // Standard COCO 80 classes used by Ultralytics YOLO (v5/v8/v11).
  ...
  if (class_id < 0 ||
      class_id >= static_cast<int>(kCocoNames.size())) {
    return "unknown";
  }
  return kCocoNames[static_cast<std::size_t>(class_id)];
}
