...
CanvasPlan ComputeCanvasPlanAndShifts(
    ...
    int* out_num_frames,
    std::vector<int>* out_shift_x,
    std::vector<int>* out_shift_y) {
  ...
  out_shift_x->assign(nframes, 0);
  out_shift_y->assign(nframes, 0);
  ...
  for (const auto& r : stable_results) {
    ...
    double dx_orig = -r.delta_x * scale_x;
    double dy_orig = -r.delta_y * scale_y;

    int sx = static_cast<int>(std::lround(dx_orig));
    int sy = static_cast<int>(std::lround(dy_orig));

    if (sx & 1) sx += (sx >= 0 ? 1 : -1);
    if (sy & 1) sy += (sy >= 0 ? 1 : -1);

    (*out_shift_x)[r.frame_idx] = sx;
    (*out_shift_y)[r.frame_idx] = sy;
  }

  int min_sx = 0, max_sx = 0;
  ...
  int canvas_w = src_w + (max_sx - min_sx);
  int canvas_h = src_h + (max_sy - min_sy);
  ...
  plan.origin_x = -min_sx;
  plan.origin_y = -min_sy;
...
void VideoExportor::WorkerMain(std::string src_video_path,
  ...
  auto CopyToCanvas = [&](const AVFrame* src, AVFrame* dst, int dst_x_y, int dst_y_y) {
    if (!src || !dst) return;

    if (dst->format == AV_PIX_FMT_YUV420P) {
      CopyRect ry = ComputeCopyRect(src_w, src_h, out_w, out_h, dst_x_y, dst_y_y);
      CopyPlaneBytes(src->data[0], src->linesize[0], dst->data[0], dst->linesize[0], 1, ry);

      const int src_w_c = src_w / 2;
      const int src_h_c = src_h / 2;
      const int out_w_c = out_w / 2;
      const int out_h_c = out_h / 2;
      const int dst_x_c = dst_x_y / 2;
      const int dst_y_c = dst_y_y / 2;

      CopyRect rc = ComputeCopyRect(src_w_c, src_h_c, out_w_c, out_h_c, dst_x_c, dst_y_c);
      CopyPlaneBytes(src->data[1], src->linesize[1], dst->data[1], dst->linesize[1], 1, rc);
      CopyPlaneBytes(src->data[2], src->linesize[2], dst->data[2], dst->linesize[2], 1, rc);
      return;
    }

    if (dst->format == AV_PIX_FMT_P010LE) {
      CopyRect ry = ComputeCopyRect(src_w, src_h, out_w, out_h, dst_x_y, dst_y_y);
      CopyPlaneBytes(src->data[0], src->linesize[0], dst->data[0], dst->linesize[0], 2, ry);
      ...
      CopyRect rc = ComputeCopyRect(src_w_c, src_h_c, out_w_c, out_h_c, dst_x_c, dst_y_c);
      CopyPlaneBytes(src->data[1], src->linesize[1], dst->data[1], dst->linesize[1], 4, rc);
      return;
    }
    ...
    if (dst->format == AV_PIX_FMT_YUV420P10LE) {
      ...
      CopyRect rc = ComputeCopyRect(src_w_c, src_h_c, out_w_c, out_h_c, dst_x_c, dst_y_c);
      CopyPlaneBytes(src->data[1], src->linesize[1], dst->data[1], dst->linesize[1], 2, rc);
      CopyPlaneBytes(src->data[2], src->linesize[2], dst->data[2], dst->linesize[2], 2, rc);
      return;
    }
  ...
  while (!stop_) {
    ...
    const int dst_x_y = plan.origin_x + sx;
    const int dst_y_y = plan.origin_y + sy;
